...
| **One flow, two connected notes** | A step can declare a **linked note**: walk the flow once and get both the literature note and the permanent note, related by *meaning* (`inspired-by`, `supports`, `contradicts` …) instead of a bare link. The wizard asks nothing extra — the pairing is part of the step — and it says where both notes will land and what the relation will be **before** writing. Create-only: an existing note is never touched. |
