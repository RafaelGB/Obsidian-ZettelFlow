...
| **Living knowledge map** | The connected **regions** of your graph, each named after its most connected note, regenerating as the vault changes. Drawn in the graph lens and readable from the `zf` scripting API. |
...
| **The graph lens** | An immersive **Knowledge Galaxy** — an interactive 3D force-directed graph of your slip-box over a starfield, and a **lens on your Explore selection**: nodes sized by connectivity and coloured by **named region** — each region a connected piece of your graph, labelled in the scene and listed in a legend you can click to fly to it — with region-hued glow halos, links coloured by relation type; search-to-focus, state filter, open it on a note and it tells you which region you landed in; a discovery lens for orphans · dead-ends · contradictions · **alone** (notes linked to nothing else in your knowledge). A one-click **cinematic tour** flies through your hubs; **share your universe** by exporting the view to a PNG or the time-lapse to a WebM clip. Read-only, offline; respects reduced-motion and Lite mode; 2D-map fallback on mobile. |
...
| **Think before you look** | Ask your vault something and answer *from memory first*. Only then does it show what you already knew, side by side with what you thought — and you say what changed in you. No tally, no accuracy, no verdict: the point is watching your own reasoning move. |
