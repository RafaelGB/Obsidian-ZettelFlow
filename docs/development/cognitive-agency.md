...
| **A wager resolved (#571)** | the claim's own verdict, recorded exactly as a return records one — subject `claim:<path>`, `confirmed` / `modified` / `rejected`. **Nothing records whether you were right**: the observation is a thought, the horizon is simply gone, and no field, flag or rate says how the prediction turned out | `derived` when the horizon brought it back, `human` when you opened the return |
| **A collision (#568)** | two notes with nothing in common that the draw put together: *nothing here* (`rejected`, subject `collision:<path>`, the pair canonicalised as #534 does it). Only *no* is recorded — answering writes a thought instead, and closing the panel records nothing | `derived` |
