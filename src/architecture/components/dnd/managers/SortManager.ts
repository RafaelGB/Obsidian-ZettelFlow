...
import { Axis, DragEventData, Entity, EntityAndElement, SiblingDirection } from "../model/archDnDModel";
import { getDropDuration } from "../DragOverlay";
import { removeStyle, setStyle, transitions } from "../utils/Animation";
import { getHitboxDimensions } from "../utils/Hitbox";
import { DRAG_LEAVE_DEBOUNCE_LENGTH, EMPTY_DIMENSIONS } from "../utils/Dimensions";
import { getSiblingDirection } from "../utils/Path";
...
export class SortManager {
    ...
    hitboxDimensions = EMPTY_DIMENSIONS;
        ...
        if (this.shifted.size > 0) {
            this.shifted.forEach((entityId) => {
                if (this.sortables.has(entityId)) {
                    const pair = this.sortables.get(entityId);
                    if (pair) {
                        const [, el] = pair;
    ...
    handleDragEnd = ({
        ...
        if (!this.isSorting || !dragPosition || !dragOriginHitbox || !dragEntity) {
            ...
            if (primaryIntersection && dragPosition) {
        ...
        this.hitboxDimensions = EMPTY_DIMENSIONS;
    };
