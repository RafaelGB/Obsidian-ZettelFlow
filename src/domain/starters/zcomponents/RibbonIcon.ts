...
export class RibbonIcon extends PluginComponent{
    public static iconId = 'zettelflow-ribbon-icon';
    ...
    onLoad(): void { 
        addIcon(RibbonIcon.iconId, this.svgContent);
        this.plugin.addRibbonIcon(RibbonIcon.iconId, this.ribbonTitle, this.ribbonIconCallback);
        log.info('RibbonIcon loaded');
    }
    private ribbonIconCallback = (evt: MouseEvent) => {
        const { options } = this.plugin.settings;
    }
}